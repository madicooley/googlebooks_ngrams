...
def read_results(query, collection, n_top_words):
    dat = {}
...
def plot(query):
    eng_c, eng_l, eng_e = read_results(query, 'English', 20)
    amer_c, amer_l, amer_e = read_results(query, 'American', 20)
    ...
    rects1 = ax.bar(x - width/2, eng_counts,  width, yerr = eng_errors, label='British English')
    rects2 = ax.bar(x + width/2, amer_counts,  width, yerr = amer_errors, label='American English')
...
def main():
    query = 'all'  # the letter of the ngram we want to read
